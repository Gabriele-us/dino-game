...
window.onload = function() {
    ...
    // Controlli Universali (Tastiera + Touch)
...
function handleInput(e) {
    if (e.type === "touchstart" && !gameOver) e.preventDefault(); // Evita scroll su mobile
    ...
    if (gameStarted) {
        // Salta se preme Spazio/FrecciaSu o se tocca lo schermo
...
function update() {
    ...
    // 1. AUMENTO VELOCITÀ OGNI 100 PUNTI
    // Aumenta di 0.4 ogni blocco di 100 punti
...
function placeClouds() {
    if (gameOver || !gameStarted) return;
    cloudArray.push({ img: cloudImg, x: boardWidth, y: Math.random() * 80 });
}

function detectCollision(a, b) {
    let pX = 12; let pY = 10; // Padding hitbox per Goku
