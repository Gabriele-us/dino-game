...
let score = 0;
let highScore = 0;
let selectedCharacter = "dino"; // Identificativo del personaggio (dino, goku o mario)
let selectedCharacterSrc = "./img/dino.png"; // Percorso immagine di default

// Personaggio (Dino / Goku / Mario)
let dinoWidth = 70; 
let dinoHeight = 80;
let dinoX = 50;
...
window.onload = function() {
    ...
    // Gestione del click sui personaggi nel Menu
    const charOptions = document.querySelectorAll(".char-option");
    charOptions.forEach(option => {
        option.addEventListener("click", function() {
            // Rimuove l'evidenziazione da tutte le opzioni
            charOptions.forEach(opt => opt.classList.remove("selected"));
            // La aggiunge solo all'opzione cliccata
            this.classList.add("selected");
            
            // Controlla l'attributo data-char per capire chi è stato scelto
            selectedCharacter = this.getAttribute("data-char");
            if (selectedCharacter === "goku") {
                selectedCharacterSrc = "./img/goku.png";
            } else if (selectedCharacter === "mario") {
                selectedCharacterSrc = "./img/super-mario.png";
            } else {
                selectedCharacterSrc = "./img/dino.png";
            }
        });
    });
    ...
    document.getElementById("play-button").addEventListener("click", function() {
        let inputName = document.getElementById("nickname").value.trim();
        
        // Se l'input è vuoto assegna il nome in base al personaggio selezionato
        if (inputName === "") {
            if (selectedCharacter === "goku") {
                playerName = "Goku";
            } else if (selectedCharacter === "mario") {
        ...
        // Assegna e carica l'immagine del personaggio scelto dall'utente
        dinoImg = new Image(); 
        dinoImg.src = selectedCharacterSrc;

        gameStarted = true;
        
        requestAnimationFrame(update);
        setInterval(placeCactus, 1000);
        setInterval(placeClouds, 2500); 
    });
...
function update() {
    ...
    // 1. Velocità
    velocityX = initialVelocityX - (Math.floor(score / 100) * 0.4); 
    ...
    // 4. PERSONAGGIO SELEZIONATO E NOME
    velocityY += gravity;
